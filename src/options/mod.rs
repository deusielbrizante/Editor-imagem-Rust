    ...
    match img {
        ...
        Err(err) => {
            match err {
                ...
                Limits(limit_err) => {
                    println!("A conclusão da operação exige mais recursos do que o permitido! {:?}", limit_err);
                    wait_enter();
                ...
                Unsupported(unsupported_err) => {
                    println!("Erro ao tentar executar a ação. Reveja a ordem das ações pedida! {:?}", unsupported_err);
                    wait_enter();
